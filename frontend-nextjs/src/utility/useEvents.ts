...
import { type ParsedEventType, getEventsData } from "./eventsUtil";
...
export function useFilteredEvents() {
	...
	const query = useQuery({
		queryKey: ["filteredEvents", timeFilteredEvents],
		queryFn: () => timeFilteredEvents,
		enabled: !isLoading && timeFilteredEvents.length > 0,
	});
